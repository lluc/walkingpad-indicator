...
À chaque déconnexion BLE, si la distance marchée ≥ 10 m, une entrée JSONL est ajoutée dans :
...
Les valeurs `distance_m` et `steps` sont des **deltas** calculés entre le snapshot initial (première notification FTMS) et les dernières valeurs reçues avant déconnexion.
